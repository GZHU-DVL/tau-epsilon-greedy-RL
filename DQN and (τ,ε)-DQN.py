...
This file contains only the code for the DQN (τ,ε)-greedy algorithm, and the complete code will be published in the future.
...
class DQN():
    ...
    def select_action_L(self, state,SINR_A,SINR_C):
        if(SINR_C - SINR_A>0) :
            self.POSAI = stats.norm.pdf(SINR_C - SINR_A, loc=0, scale=0.8)
        else:
    ...
    def optimize_model(self, state, next_state, action, R, BATCH_SIZE=10):
        ...
        next_state_action_values = self.model(next_states).max(1)[0]  
